...
function bind_comment_form()
{
	...
	$(".replycomment").bind("click",function(){
		if($(this).parent().parent().parent().parent().find(".reply_box").css("display")=="none"){
			$(this).parent().parent().parent().parent().find(".reply_box").show();
		}else{
			$(this).parent().parent().parent().parent().find(".reply_box").hide();
		}
	});
	...
	$(".send_btn").bind("click",function(){
		...
		if($(form).find(".send_btn").html()=="发送中")
		{
			return false;
		}
		var ajaxurl = $(form).attr("action");
		var query = $(form).serialize() ;
		$(form).find(".send_btn").html("发送中");
		$.ajax({
			url: ajaxurl,
			dataType: "json",
			data:query,
