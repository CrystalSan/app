...
function get_user_avatar($id,$type)
{
    ...
    if(file_exists($avatar_check_file)){
        ...
        //检查用户是否有绑定微信
        $idx = $GLOBALS['db']->getRow("select * from  ".DB_PREFIX."user_idx where userid=".$uid);
        //检查用户是否有微信头像
        $pic=$GLOBALS['db']->getOne("select `headimgurl` from  ".DB_PREFIX."user where id=".$uid);
        if($pic!==null&&@fopen($pic,'r')&&$idx){//如果有绑定微信&头像地址有效
            return $pic;
        }else{
...
function send_deal_success_1(){
	...
	foreach ($deal_s_user as $k=>$v){
		if($v['id']&&$v['mobile']){
			...
			$msg_data3['content']=$v['user_name'].'|'.$v['name'];
	...
	if($success_deal_user){
		...
		foreach ($success_deal_user as $k=>$v){
			if($v['id']&&$v['mobile']){
				...
				$msg_data['content']=$v['user_name'].'|'.$v['deal_name'];
...
function send_deal_fail_1(){
	...
	foreach ($deal_f_user as $k=>$v){
		if($v['id']&&$v['mobile']){
			...
			$msg_data2['content']=$v['user_name'].'|'.$v['name'];
