...
class ajaxModule extends BaseModule
{
	...
	function leader_detailed_information(){
		...
		if($id>0){
			//领投信息
			$leader_info=$GLOBALS['db']->getRow("select inv.*,u.user_name,u.identify_name from  ".DB_PREFIX."investment_list as inv left join ".DB_PREFIX."user as u on u.id=inv.user_id where inv.id=".$id);
	...
	function download(){
		$filename =unserialize($_REQUEST['leader_moban']);
		if($filename==null){
			showErr("文件不存在！");
			return false;
		}else{
	...
	public function check_aqj_login()
	{
		$log_mobile = trim($_REQUEST['log_mobile']);
		$log_pwd = trim($_REQUEST['log_pwd']);
		$xlc_user=$GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where mobile='".$log_mobile."' and user_pwd='".md5($log_pwd)."'");
		$result = array("status"=>1,"info"=>'');
		if($xlc_user)
		{
			ajax_return($result);
