...
require APP_ROOT_PATH.'app/Lib/shop_lip.php';
 class apModule extends BaseModule
{
	...
	public function init_deal_page($deal_info)
	{
		...
		foreach ($deal_item_list as $k=>$v){
			...
			$deal_item_list[$k]['ap_price']=$v['price']*$v['ap_ratio'];
		}
	...
	public function getTopicList()
	{
		$p = intval($_REQUEST['p']);
		$limit = " ".($p*3).",3";
		$all_topic_num = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."zhuanti where 1 ");
		$topic_list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."zhuanti where 1 order by create_time desc limit ".$limit);
		if(!$topic_list)
		{
			$data['status'] = 3;	
			$data['info'] = "没有更多专题";
			...
			$data['status'] = 1;
			$data['info'] = json_encode($topic_list);
		}
		if($all_topic_num > ($p*3+3)){
			$data['more'] = 1;
		}else{
			$data['more'] = 2;
		}
